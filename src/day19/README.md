...
robot_ in your pack that you can use to <span title="If You Give A Mouse An Ore-Collecting Robot">kickstart</span> the
whole operation.
...
Using blueprint 1 in the example above, the largest number of geodes you could open in 24 minutes is _`9`_. One way to
achieve that is:
...
9 geodes, so its quality level is _`9`_. The second blueprint has ID 2 and can open 12 geodes, so its quality level is
_`24`_. Finally, if you _add up the quality levels_ of all of the blueprints in the list, you get _`33`_.
...
In 32 minutes, the largest number of geodes blueprint 1 (from the example above) can open is _`56`_. One way to achieve
that is:
